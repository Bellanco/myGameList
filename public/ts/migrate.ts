// @ts-nocheck
/**
 * MIGRATE.TS — Conversión de formato heredado a nomenclatura oficial
...
function migrateGame(game: any, tab: string): any {
...
export function migrateData(data: any): any {
  ...
  return {
    c: (Array.isArray(data.c) ? data.c : []).map((g: any) => migrateGame(g, 'c')),
    v: (Array.isArray(data.v) ? data.v : []).map((g: any) => migrateGame(g, 'v')),
    e: (Array.isArray(data.e) ? data.e : []).map((g: any) => migrateGame(g, 'e')),
    p: (Array.isArray(data.p) ? data.p : []).map((g: any) => migrateGame(g, 'p')),

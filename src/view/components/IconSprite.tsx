export function IconSprite() {
  return (
    <svg aria-hidden="true" className="svg-sprite">
      {/* FILTROS DE TEXTURA. Van aquí y no en una hoja porque un `filter: url(#id)` de CSS necesita que la
          definición exista EN EL DOCUMENTO, y este sprite ya se monta una vez en la raíz. No pintan nada por
          sí mismos: los usa el skin de «Plata y acero» para desgarrar el canto de sus carteles de contrato,
          con ruido fractal desplazando el borde (`feTurbulence` + `feDisplacementMap`), de modo que ningún
          cartel se rompe igual que otro y el desgarro no se repite nunca. */}
      <defs>
        {/* CUÁNTOS CORTES CABEN EN UN CANTO DE 70 px. A 0,055 el ruido tiene un periodo de ~18 px, o sea
            tres ondas en un cartel de género y ninguna entera en uno de año («2012», 34 px): el papel
            salía casi recto justo en los carteles pequeños, que son la mayoría de la tabla. A 0,095 el
            periodo baja a ~10 px y cada cartel enseña dos o tres roturas. El desplazamiento baja a la vez
            de 6,5 a 6 px: más cortes con la misma amplitud empieza a leerse como sierra, y lo que se busca
            es papel roto. La quinta octava tampoco hacía falta —a este tamaño sólo añadía polvo—. */}
        <filter id="wt-desgarro">
          <feTurbulence type="fractalNoise" baseFrequency="0.095" numOctaves="4" seed="6" result="ruido" />
          <feDisplacementMap in="SourceGraphic" in2="ruido" scale="6" xChannelSelector="R" yChannelSelector="G" />
        </filter>
        {/* EL MISMO ROTO, PARA UNA PIEZA GRANDE. El de arriba desplaza el borde 6 px, que es lo que
            rompe el canto de un cartel de 20 px de alto; en el pliego de una caja del mosaico —218 px—
            ese mismo desplazamiento es el mismo roto repartido en diez veces más borde, y el papel sale
            casi recto. Este baja la frecuencia del ruido (dientes más largos) y sube el desplazamiento,
            de modo que el pliego se rasga a la escala a la que se ve. Lo usa la caja sin carátula de
            «Plata y acero». */}
        <filter id="wt-desgarro-pliego">
          <feTurbulence type="fractalNoise" baseFrequency="0.028 0.042" numOctaves="4" seed="11" result="ruido" />
          <feDisplacementMap in="SourceGraphic" in2="ruido" scale="15" xChannelSelector="R" yChannelSelector="G" />
        </filter>
      </defs>
      <symbol id="icon-plus" viewBox="0 0 24 24"><path d="M11 5h2v14h-2zM5 11h14v2H5z" /></symbol>
      <symbol id="icon-check" viewBox="0 0 24 24"><path d="M9 16.17 4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.42z" /></symbol>
      <symbol id="icon-download" viewBox="0 0 24 24"><path d="M5 20h14v-2H5v2zm7-18v10.17l3.59-3.58L17 10l-5 5-5-5 1.41-1.41L11 12.17V2h1z" /></symbol>
      <symbol id="icon-upload" viewBox="0 0 24 24"><path d="M5 20h14v-2H5v2zm7-18 5 5-1.41 1.41L13 5.83V16h-2V5.83L8.41 8.41 7 7l5-5z" /></symbol>
      
      <symbol id="icon-edit" viewBox="0 -960 960 960"><path d="M200-120q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h357l-80 80H200v560h560v-278l80-80v358q0 33-23.5 56.5T760-120H200Zm280-360ZM360-360v-170l367-367q12-12 27-18t30-6q16 0 30.5 6t26.5 18l56 57q11 12 17 26.5t6 29.5q0 15-5.5 29.5T897-728L530-360H360Zm481-424-56-56 56 56ZM440-440h56l232-232-28-28-29-28-231 231v57Zm260-260-29-28 29 28 28 28-28-28Z" /></symbol>
      <symbol id="icon-trash" viewBox="0 -960 960 960"><path d="M280-120q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520ZM360-280h80v-360h-80v360Zm160 0h80v-360h-80v360ZM280-720v520-520Z" /></symbol>
      <symbol id="icon-close" viewBox="0 0 24 24"><path d="M18.3 5.7 12 12l6.3 6.3-1.4 1.4L10.6 13.4 4.3 19.7 2.9 18.3 9.2 12 2.9 5.7 4.3 4.3l6.3 6.3 6.3-6.3 1.4 1.4z" /></symbol>            {/* Compartir (Material Symbols Outlined «share», FILL 0 · peso 400 · grado 0 · tamaño óptico 24).
          Es la misma familia que la mayoría del sprite —de ahí el viewBox 0 -960 960 960— y va bajo Apache 2.0,
          sin la atadura de atribución que tienen los iconos de Font Awesome. */}
      <symbol id="icon-share-nodes" viewBox="0 -960 960 960"><path d="M680-80q-50 0-85-35t-35-85q0-6 3-28L282-392q-16 15-37 23.5t-45 8.5q-50 0-85-35t-35-85q0-50 35-85t85-35q24 0 45 8.5t37 23.5l281-164q-2-7-2.5-13.5T560-760q0-50 35-85t85-35q50 0 85 35t35 85q0 50-35 85t-85 35q-24 0-45-8.5T598-672L317-508q2 7 2.5 13.5t.5 14.5q0 8-.5 14.5T317-452l281 164q16-15 37-23.5t45-8.5q50 0 85 35t35 85q0 50-35 85t-85 35Zm0-80q17 0 28.5-11.5T720-200q0-17-11.5-28.5T680-240q-17 0-28.5 11.5T640-200q0 17 11.5 28.5T680-160ZM200-440q17 0 28.5-11.5T240-480q0-17-11.5-28.5T200-520q-17 0-28.5 11.5T160-480q0 17 11.5 28.5T200-440Zm508.5-291.5Q720-743 720-760t-11.5-28.5Q697-800 680-800t-28.5 11.5Q640-777 640-760t11.5 28.5Q663-720 680-720t28.5-11.5ZM680-200ZM200-480Zm480-280Z" /></symbol>
      <symbol id="icon-angle-right" viewBox="0 0 320 512"><path d="M278.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-160 160c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L210.7 256 73.4 118.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l160 160z" /></symbol>
      <symbol id="icon-refresh" viewBox="0 0 24 24"><path d="M17.65 6.35A7.958 7.958 0 0012 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08A5.99 5.99 0 0112 18c-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z" /></symbol>      <symbol id="icon-filter" viewBox="0 -960 960 960"><path d="M400-240v-80h160v80H400ZM240-440v-80h480v80H240ZM120-640v-80h720v80H120Z" /></symbol>
      <symbol id="icon-filter-active" viewBox="0 -960 960 960"><path d="M400-240v-80h160v80H400ZM240-440v-80h480v80H240ZM120-640v-80h720v80H120Z" /><circle cx="600" cy="200" r="40" fill="currentColor" opacity="0.3" /></symbol>
      <symbol id="icon-steamdeck" viewBox="0 0 24 24">
        <title>Steam Deck</title>
        <path d="M8.494 0v4.309c4.242 0 7.694 3.45 7.694 7.691s-3.452 7.691-7.694 7.691V24c6.617 0 12-5.383 12-12s-5.383-12-12-12zm10.819 3.62v.194h.328v.893h.228v-.893h.33V3.62zm1.037 0v1.087h.207v-.684l.298.653h.14l.29-.66v.691h.219V3.619h-.23l-.338.772-.368-.772zM8.494 6.011a5.998 5.998 0 0 0-5.998 6 5.998 5.998 0 1 0 11.998 0 6 6 0 0 0-6-6z" />
      </symbol>
      <symbol id="icon-repeat" viewBox="0 0 24 24"><path d="M7 7h10V4l5 4-5 4V9H7a3 3 0 00-3 3v1H2v-1a5 5 0 015-5zm10 10H7v3l-5-4 5-4v3h10a3 3 0 003-3v-1h2v1a5 5 0 01-5 5z" /></symbol>
      <symbol id="icon-undo" viewBox="0 0 24 24"><path d="M12 5a8 8 0 00-8 8H1l4 4 4-4H6a6 6 0 116 6v2a8 8 0 000-16z" /></symbol>      
      <symbol id="icon-star" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></symbol>
      <symbol id="icon-star-olive-branches" viewBox="0 0 32.148 32.148">
        <path d="M3.171,19.56c0,0,0.313-1.268-0.739-2.367c-1.056-1.101-2.382-1.105-2.382-1.105s-0.33,1.489,0.698,2.562 C1.775,19.72,3.171,19.56,3.171,19.56z" />
        <path d="M5.5,19.33c0,0,1.233-0.669,1.459-2.139c0.227-1.466-0.9-2.498-0.9-2.498s-1.08,0.768-1.313,2.274 C4.515,18.474,5.5,19.33,5.5,19.33z" />
        <path d="M7.354,23.029c0,0,1.136-0.827,1.169-2.312c0.03-1.483-1.221-2.358-1.221-2.358s-0.971,0.9-1.004,2.425 C6.265,22.308,7.354,23.029,7.354,23.029z" />
        <path d="M11.076,22.754c-0.108-1.479-1.436-2.233-1.436-2.233s-0.882,0.987-0.772,2.508c0.11,1.521,1.262,2.136,1.262,2.136 S11.183,24.234,11.076,22.754z" />
        <path d="M13.617,27.009c0,0,0.743-1.191,0.214-2.58c-0.529-1.388-2.019-1.728-2.019-1.728s-0.561,1.202-0.019,2.627 C12.337,26.751,13.617,27.009,13.617,27.009z" />
        <path d="M3.685,16.312c0,0,0.966-0.879,0.699-2.379c-0.267-1.499-1.366-2.242-1.366-2.242s-1.101,1.058-0.841,2.521 C2.437,15.672,3.685,16.312,3.685,16.312z" />
        <path d="M4.443,22.437c0,0-0.074-1.305-1.404-2.045c-1.333-0.738-2.602-0.355-2.602-0.355s0.124,1.521,1.422,2.242 C3.159,23,4.443,22.437,4.443,22.437z" />
        <path d="M6.688,24.969c0,0-0.362-1.255-1.823-1.681c-1.464-0.426-2.616,0.23-2.616,0.23s0.461,1.455,1.886,1.871 C5.562,25.806,6.688,24.969,6.688,24.969z" />
        <path d="M9.199,26.773c0,0-0.724-1.086-2.246-1.049c-1.524,0.037-2.424,1.01-2.424,1.01s0.88,1.25,2.363,1.213 C8.377,27.913,9.199,26.773,9.199,26.773z" />
        <path d="M10.271,27.509c-1.508,0.221-2.284,1.294-2.284,1.294s1.021,1.136,2.491,0.923c1.471-0.214,2.148-1.442,2.148-1.442 S11.778,27.292,10.271,27.509z" />
        <path d="M4.182,17.058l-0.727-0.029c-0.002,0.08-0.183,7.955,11.7,11.505l0.208-0.696C4.048,24.457,4.169,17.357,4.182,17.058z" />
        <path d="M28.977,19.581c0,0,1.396,0.16,2.423-0.912c1.028-1.072,0.698-2.562,0.698-2.562s-1.326,0.006-2.383,1.106 C28.662,18.314,28.977,19.581,28.977,19.581z" />
        <path d="M25.188,17.212c0.226,1.469,1.459,2.139,1.459,2.139s0.984-0.855,0.755-2.361c-0.233-1.507-1.313-2.274-1.313-2.274 S24.962,15.746,25.188,17.212z" />
        <path d="M24.846,18.379c0,0-1.251,0.875-1.222,2.358c0.033,1.483,1.169,2.312,1.169,2.312s1.089-0.722,1.057-2.245 C25.816,19.282,24.846,18.379,24.846,18.379z" />
        <path d="M21.071,22.775c-0.106,1.479,0.946,2.408,0.946,2.408s1.15-0.613,1.262-2.135c0.109-1.521-0.772-2.509-0.772-2.509 S21.181,21.294,21.071,22.775z" />
        <path d="M18.316,24.45c-0.529,1.388,0.214,2.579,0.214,2.579s1.279-0.258,1.823-1.68c0.542-1.425-0.02-2.628-0.02-2.628 S18.847,23.062,18.316,24.45z" />
        <path d="M28.463,16.333c0,0,1.248-0.64,1.509-2.101c0.26-1.463-0.842-2.521-0.842-2.521s-1.1,0.743-1.365,2.242 C27.497,15.454,28.463,16.333,28.463,16.333z" />
        <path d="M29.107,20.413c-1.33,0.739-1.404,2.044-1.404,2.044s1.285,0.564,2.584-0.157c1.298-0.721,1.422-2.243,1.422-2.243 S30.44,19.673,29.107,20.413z" />
        <path d="M27.281,23.31c-1.461,0.426-1.823,1.681-1.823,1.681s1.128,0.836,2.555,0.421c1.425-0.415,1.886-1.871,1.886-1.871 S28.745,22.883,27.281,23.31z" />
        <path d="M25.194,25.745c-1.522-0.037-2.246,1.05-2.246,1.05s0.821,1.139,2.307,1.174c1.483,0.036,2.363-1.213,2.363-1.213 S26.72,25.783,25.194,25.745z" />
        <path d="M21.876,27.529c-1.508-0.217-2.354,0.773-2.354,0.773s0.678,1.229,2.147,1.443c1.471,0.213,2.491-0.924,2.491-0.924 S23.384,27.75,21.876,27.529z" />
        <path d="M28.692,17.048l-0.728,0.029c0.013,0.301,0.134,7.398-11.181,10.779l0.208,0.695 C28.875,25.003,28.694,17.128,28.692,17.048z" />
        <polygon points="21.771,20.214 20.945,13.495 25.443,8.924 18.996,8.062 16.076,2.404 16.076,2.376 13.15,8.042 6.704,8.903 11.202,13.475 10.377,20.193 16.071,17.099 16.071,17.117" />
      </symbol>
      <symbol id="icon-lock" viewBox="0 0 24 24"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5s-5 2.24-5 5v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zm3.1-9H8.9V6c0-1.71 1.39-3.1 3.1-3.1 1.71 0 3.1 1.39 3.1 3.1v2z" /></symbol>
      <symbol id="icon-skull" viewBox="0 0 24 24"><path fillRule="evenodd" clipRule="evenodd" d="M12 2.2C7.4 2.2 3.6 5.8 3.6 10.4c0 2.6 1.3 4.8 3.3 6.3.6.4 1 1.2 1 1.9v.6c0 1 .8 1.8 1.8 1.8h.1v-1.6c0-.4.3-.7.7-.7s.7.3.7.7v1.6h1.4v-1.6c0-.4.3-.7.7-.7s.7.3.7.7v1.6h.1c1 0 1.8-.8 1.8-1.8v-.6c0-.7.4-1.5 1-1.9 2-1.5 3.3-3.7 3.3-6.3 0-4.6-3.8-8.2-8.4-8.2Zm-3.4 6.2c1.3 0 2.3 1.1 2.3 2.4S9.8 13.2 8.6 13.2 6.3 12.1 6.3 10.8 7.3 8.4 8.6 8.4Zm6.8 0c1.3 0 2.3 1.1 2.3 2.4s-1 2.4-2.3 2.4-2.3-1.1-2.3-2.4 1-2.4 2.3-2.4ZM12 13.2l1.1 2.3h-2.2L12 13.2Z" /></symbol>
      <symbol id="icon-rocket" viewBox="0 0 24 24">
        <path fillRule="evenodd" clipRule="evenodd" d="M12 2.2c2.9 2.4 4.4 5.7 4.4 9.2 0 1.7-.35 3.4-1.05 5H8.65C7.95 14.8 7.6 13.1 7.6 11.4c0-3.5 1.5-6.8 4.4-9.2Zm0 4.8a1.8 1.8 0 100 3.6 1.8 1.8 0 000-3.6Z" />
        <path d="M7.4 12.5c-1.9.9-3.1 2.7-3.3 4.9l3.3-1.5v-3.4Z" />
        <path d="M16.6 12.5c1.9.9 3.1 2.7 3.3 4.9l-3.3-1.5v-3.4Z" />
        <path d="M10.1 17.6h3.8L12 21.6l-1.9-4Z" />
      </symbol>
      <symbol id="icon-trophy" viewBox="0 -960 960 960"><path d="M280-880h400v314q0 23-10 41t-28 29l-142 84 28 92h152l-124 88 48 152-124-94-124 94 48-152-124-88h152l28-92-142-84q-18-11-28-29t-10-41v-314Zm80 80v234l80 48v-282h-80Zm240 0h-80v282l80-48v-234ZM480-647Zm-40-12Zm80 0Z" /></symbol>
      <symbol id="icon-play" viewBox="0 -960 960 960"><path d="M480-654Zm174 174Zm-348 0Zm174 174Zm0-234L360-660v-220h240v220L480-540Zm180 180L540-480l120-120h220v240H660Zm-580 0v-240h220l120 120-120 120H80ZM360-80v-220l120-120 120 120v220H360Zm120-574 40-40v-106h-80v106l40 40ZM160-440h106l40-40-40-40H160v80Zm280 280h80v-106l-40-40-40 40v106Zm254-280h106v-80H694l-40 40 40 40Z" /></symbol>
      <symbol id="icon-abandoned" viewBox="0 -960 960 960"><path d="M440-200q-100 0-170-70t-70-170q0-11 1-22t3-22q-5 2-12 3t-12 1q-42 0-71-29t-29-71q0-42 27.5-71t69.5-29q33 0 59.5 18.5T274-614q33-30 75.5-48t90.5-18h440v160H680v80q0 100-70 170t-170 70ZM208.5-551.5Q220-563 220-580t-11.5-28.5Q197-620 180-620t-28.5 11.5Q140-597 140-580t11.5 28.5Q163-540 180-540t28.5-11.5ZM539-341q41-41 41-99t-41-99q-41-41-99-41t-99 41q-41 41-41 99t41 99q41 41 99 41t99-41Zm-42.5-42.5Q520-407 520-440t-23.5-56.5Q473-520 440-520t-56.5 23.5Q360-473 360-440t23.5 56.5Q407-360 440-360t56.5-23.5ZM440-440Z" /></symbol>
      <symbol id="icon-checkered-flag" viewBox="0 -960 960 960"><path d="M360-720h80v-80h-80v80Zm160 0v-80h80v80h-80ZM360-400v-80h80v80h-80Zm320-160v-80h80v80h-80Zm0 160v-80h80v80h-80Zm-160 0v-80h80v80h-80Zm160-320v-80h80v80h-80Zm-240 80v-80h80v80h-80ZM200-160v-640h80v80h80v80h-80v80h80v80h-80v320h-80Zm400-320v-80h80v80h-80Zm-160 0v-80h80v80h-80Zm-80-80v-80h80v80h-80Zm160 0v-80h80v80h-80Zm80-80v-80h80v80h-80Z"/></symbol>
      
      <symbol id="icon-chevron-up" viewBox="0 0 512 512"><path d="M233.4 105.4c12.5-12.5 32.8-12.5 45.3 0l192 192c12.5 12.5 12.5 32.8 0 45.3s-32.8 12.5-45.3 0L256 173.3 86.6 342.6c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3l192-192z"/></symbol>
      <symbol id="icon-dice-d20" viewBox="0 0 512 512"><path d="M48.7 125.8l53.2 31.9c7.8 4.7 17.8 2 22.2-5.9L201.6 12.1c3-5.4-.9-12.1-7.1-12.1c-1.6 0-3.2 .5-4.6 1.4L47.9 98.8c-9.6 6.6-9.2 20.9 .8 26.9zM16 171.7l0 123.5c0 8 10.4 11 14.7 4.4l60-92c5-7.6 2.6-17.8-5.2-22.5L40.2 158C29.6 151.6 16 159.3 16 171.7zM310.4 12.1l77.6 139.6c4.4 7.9 14.5 10.6 22.2 5.9l53.2-31.9c10-6 10.4-20.3 .8-26.9L322.1 1.4c-1.4-.9-3-1.4-4.6-1.4c-6.2 0-10.1 6.7-7.1 12.1zM496 171.7c0-12.4-13.6-20.1-24.2-13.7l-45.3 27.2c-7.8 4.7-10.1 14.9-5.2 22.5l60 92c4.3 6.7 14.7 3.6 14.7-4.4l0-123.5zm-49.3 246L286.1 436.6c-8.1 .9-14.1 7.8-14.1 15.9l0 52.8c0 3.7 3 6.8 6.8 6.8c.8 0 1.6-.1 2.4-.4l172.7-64c6.1-2.2 10.1-8 10.1-14.5c0-9.3-8.1-16.5-17.3-15.4zM233.2 512c3.7 0 6.8-3 6.8-6.8l0-52.6c0-8.1-6.1-14.9-14.1-15.9l-160.6-19c-9.2-1.1-17.3 6.1-17.3 15.4c0 6.5 4 12.3 10.1 14.5l172.7 64c.8 .3 1.6 .4 2.4 .4zM41.7 382.9l170.9 20.2c7.8 .9 13.4-7.5 9.5-14.3l-85.7-150c-5.9-10.4-20.7-10.8-27.3-.8L30.2 358.2c-6.5 9.9-.3 23.3 11.5 24.7zm439.6-24.8L402.9 238.1c-6.5-10-21.4-9.6-27.3 .8L290.2 388.5c-3.9 6.8 1.6 15.2 9.5 14.3l170.1-20c11.8-1.4 18-14.7 11.5-24.6zm-216.9 11l78.4-137.2c6.1-10.7-1.6-23.9-13.9-23.9l-145.7 0c-12.3 0-20 13.3-13.9 23.9l78.4 137.2c3.7 6.4 13 6.4 16.7 0zM174.4 176l163.2 0c12.2 0 19.9-13.1 14-23.8l-80-144c-2.8-5.1-8.2-8.2-14-8.2l-3.2 0c-5.8 0-11.2 3.2-14 8.2l-80 144c-5.9 10.7 1.8 23.8 14 23.8z"/></symbol>
      {/* «Flechas hacia el ojo» (Font Awesome Free 7, classic SOLID): el enlace del detalle que lleva al
          análisis completo. Dice lo que hace —concentrar la mirada en una cosa— mejor que el ojo a secas, que
          en esta app ya significa «mostrar/ocultar».
          Misma procedencia y misma obligación que el caballo de ajedrez de arriba: CC BY 4.0, con su cita en
          `core/constants/legalContent` (sección «Licencia del software»). */}
      <symbol id="icon-arrows-to-eye" viewBox="0 0 640 512"><path d="M176 56c0-13.3-10.7-24-24-24s-24 10.7-24 24l0 38.1-63-63c-9.4-9.4-24.6-9.4-33.9 0S21.7 55.6 31 65l63 63-38.1 0c-13.3 0-24 10.7-24 24s10.7 24 24 24l96 0c13.3 0 24-10.7 24-24l0-96zm0 400l0-96c0-13.3-10.7-24-24-24l-96 0c-13.3 0-24 10.7-24 24s10.7 24 24 24l38.1 0-63 63c-9.4 9.4-9.4 24.6 0 33.9s24.6 9.4 33.9 0l63-63 0 38.1c0 13.3 10.7 24 24 24s24-10.7 24-24zm312 24c13.3 0 24-10.7 24-24l0-38.1 63 63c9.4 9.4 24.6 9.4 33.9 0s9.4-24.6 0-33.9l-63-63 38.1 0c13.3 0 24-10.7 24-24s-10.7-24-24-24l-96 0c-13.3 0-24 10.7-24 24l0 96c0 13.3 10.7 24 24 24zM464 56l0 96c0 13.3 10.7 24 24 24l96 0c13.3 0 24-10.7 24-24s-10.7-24-24-24l-38.1 0 63-63c9.4-9.4 9.4-24.6 0-33.9s-24.6-9.4-33.9 0l-63 63 0-38.1c0-13.3-10.7-24-24-24s-24 10.7-24 24zM320 120c-57.3 0-99.4 34-125.6 64.3-20 23.1-32.4 45.8-37.6 56.1-2.5 5-4.9 9.7-4.9 15.6s2.3 10.6 4.9 15.6c5.2 10.3 17.6 33 37.6 56.1 26.2 30.3 68.2 64.3 125.6 64.3s99.4-34 125.6-64.3c20-23.1 32.4-45.8 37.6-56.1 2.5-5 4.9-9.7 4.9-15.6s-2.3-10.6-4.9-15.6c-5.2-10.3-17.6-33-37.6-56.1-26.2-30.3-68.2-64.3-125.6-64.3zM256 256a64 64 0 1 1 128 0 64 64 0 1 1 -128 0z" /></symbol>
      <symbol id="icon-signature" viewBox="0 0 640 512"><path d="M192 128c0-17.7 14.3-32 32-32s32 14.3 32 32l0 7.8c0 27.7-2.4 55.3-7.1 82.5l-84.4 25.3c-40.6 12.2-68.4 49.6-68.4 92l0 71.9c0 40 32.5 72.5 72.5 72.5c26 0 50-13.9 62.9-36.5l13.9-24.3c26.8-47 46.5-97.7 58.4-150.5l94.4-28.3-12.5 37.5c-3.3 9.8-1.6 20.5 4.4 28.8s15.7 13.3 26 13.3l128 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-83.6 0 18-53.9c3.8-11.3 .9-23.8-7.4-32.4s-20.7-11.8-32.2-8.4L316.4 198.1c2.4-20.7 3.6-41.4 3.6-62.3l0-7.8c0-53-43-96-96-96s-96 43-96 96l0 32c0 17.7 14.3 32 32 32s32-14.3 32-32l0-32zm-9.2 177l49-14.7c-10.4 33.8-24.5 66.4-42.1 97.2l-13.9 24.3c-1.5 2.6-4.3 4.3-7.4 4.3c-4.7 0-8.5-3.8-8.5-8.5l0-71.9c0-14.1 9.3-26.6 22.8-30.7zM24 368c-13.3 0-24 10.7-24 24s10.7 24 24 24l40.3 0c-.2-2.8-.3-5.6-.3-8.5L64 368l-40 0zm592 48c13.3 0 24-10.7 24-24s-10.7-24-24-24l-310.1 0c-6.7 16.3-14.2 32.3-22.3 48L616 416z"/></symbol>
      {/* F5 — las dos FORMAS del listado, dibujadas con lo que significan: tres renglones y cuatro cajas.
          Trazo en `currentColor` para que sigan al color del botón (activo = acento del tema). */}
      <symbol id="icon-view-list" viewBox="0 0 24 24">
        <path fill="currentColor" d="M3 5h18v3H3zm0 5.5h18v3H3zM3 16h18v3H3z" />
      </symbol>
      <symbol id="icon-view-grid" viewBox="0 0 24 24">
        <path fill="currentColor" d="M3 3h8v8H3zm10 0h8v8h-8zM3 13h8v8H3zm10 0h8v8h-8z" />
      </symbol>
      <symbol id="icon-bottom-lists" viewBox="0 0 24 24">
        <path fill="currentColor" d="M7 1c-1.1 0-2 .9-2 2v18a2 2 0 0 0 2 2h7c2.76 0 5-2.24 5-5V3a2 2 0 0 0-2-2zm1 3h8v7H8zm1 10h1v2h2v1h-2v2H9v-2H7v-1h2zm7 1c.55 0 1 .45 1 1s-.45 1-1 1s-1-.45-1-1s.45-1 1-1m-2 2c.55 0 1 .45 1 1s-.45 1-1 1s-1-.45-1-1s.45-1 1-1" />
      </symbol>
      {/* Pestaña "Ajustes" — COMPÁS DE DIBUJO (Font Awesome Free 7, classic SOLID).
          Sustituye al engranaje, que era el icono de un botón flotante suelto; la pestaña reúne ahora todo lo
          que se ajusta, y un compás dice «esto se calibra a tu medida» sin prometer una sola tuerca. Es sólido,
          como sus tres vecinos de la barra (`bottom-lists`, `bottom-hub`, `bottom-stats`); el de cuenta es el
          único a trazo.
          ATRIBUCIÓN: CC BY 4.0, con su cita en `core/constants/legalContent` (sección «Licencia del software»),
          igual que el caballo de ajedrez y las flechas hacia el ojo. El `viewBox` es el del fichero —aquí el
          trazado SÍ cabe en su caja, a diferencia del caballo— y el `fill` se hereda de `currentColor`. */}
      <symbol id="icon-bottom-settings" viewBox="0 0 512 512">
        <path fill="currentColor" d="M99.3 256.1l69.4-119.9c-5.6-12.2-8.8-25.8-8.8-40.2 0-53 43-96 96-96s96 43 96 96c0 14.3-3.1 27.9-8.8 40.2l44.4 76.7c-23.1 26-53.7 45.1-88.4 53.8L256 191.9 187.9 309.5c21.5 6.8 44.3 10.5 68.1 10.5 70.7 0 133.8-32.7 174.9-84 11.1-13.8 31.2-16 45-5s16 31.2 5 45c-52.7 65.8-133.9 108-224.8 108-35.4 0-69.4-6.4-100.7-18.1L98.7 463.7C94 471.8 87 478.4 78.6 482.6L23.2 510.3c-5 2.5-10.9 2.2-15.6-.7S0 501.5 0 496l0-55.4c0-8.4 2.2-16.7 6.5-24.1l60-103.7c-12.8-11.2-24.6-23.5-35.3-36.8-11.1-13.8-8.8-33.9 5-45s33.9-8.8 45 5c5.7 7.1 11.8 13.8 18.2 20.1zM381.1 407.9c32.5-13 62.4-31 88.9-52.9l35.6 61.5c4.2 7.3 6.5 15.6 6.5 24.1l0 55.4c0 5.5-2.9 10.7-7.6 13.6s-10.6 3.2-15.6 .7l-55.4-27.7c-8.4-4.2-15.4-10.8-20.1-18.9l-32.3-55.8zM256 128a32 32 0 1 0 0-64 32 32 0 1 0 0 64z" />
      </symbol>
      <symbol id="icon-bottom-hub" viewBox="0 0 32 32">
        <path fill="currentColor" d="M24.219 10.573V6.781a2.93 2.93 0 0 0 1.688-2.641v-.089a2.93 2.93 0 0 0-2.922-2.922h-.089a2.93 2.93 0 0 0-2.927 2.922v.089a2.93 2.93 0 0 0 1.672 2.63l.016.01v3.802a8.27 8.27 0 0 0-3.958 1.745l.016-.01l-10.438-8.13C8.22.666 3.626-1.589 1.418 1.312C-.796 4.208 2.585 8.041 5.736 6.208l-.016.01l10.26 7.984a8.26 8.26 0 0 0-1.385 4.599c0 1.786.568 3.448 1.531 4.807l-.016-.026l-3.125 3.12c-.25-.078-.51-.12-.771-.125h-.005c-2.411 0-3.625 2.922-1.917 4.63c1.708 1.703 4.63.495 4.63-1.917a2.8 2.8 0 0 0-.135-.797l.005.021l3.089-3.089a8.34 8.34 0 0 0 7.172 1.438a8.335 8.335 0 0 0 5.182-12.073a8.32 8.32 0 0 0-5.974-4.208l-.052-.01zm-1.287 12.505c-3.807-.01-5.703-4.615-3.005-7.302c2.693-2.688 7.292-.781 7.292 3.026v.005a4.27 4.27 0 0 1-4.276 4.271z" />
      </symbol>
      {/* Pestaña "Estadísticas": una tarta con su porción destacada. No una silueta —esa ya es
          la silueta de `person`— ni barras, que se confundían con el icono de las listas. El sector separado se
          dibuja como una porción entera (dos radios y su arco) desplazada, no como un trozo recortado. */}
      <symbol id="icon-bottom-stats" viewBox="0 0 24 24">
        <g fill="currentColor">
          <path d="M11.1 3.15a8.85 8.85 0 1 0 9.4 12.05l-8.2-3.4a1 1 0 0 1-.6-.92z" opacity=".55" />
          <path d="M13.6 2.4a1 1 0 0 0-1.2.98v7.02a1 1 0 0 0 .62.92l6.5 2.7a1 1 0 0 0 1.34-.6 9.2 9.2 0 0 0-7.26-11.02z" />
        </g>
      </symbol>
      <symbol id="icon-logout" viewBox="0 -960 960 960">
        <path d="m696-80-56-56 84-84-84-84 56-56 84 84 84-84 56 56-83 84 83 84-56 56-84-83-84 83Zm-216 0q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 10-.5 20t-1.5 20h-81q2-10 2.5-20t.5-20q0-20-2.5-40t-7.5-40H654q3 20 4.5 40t1.5 40v20q0 10-1 20h-80q1-10 1-20v-20q0-20-1.5-40t-4.5-40H386q-3 20-4.5 40t-1.5 40q0 20 1.5 40t4.5 40h174v80H404q12 43 31 82.5t45 75.5q18 0 35.5-2t35.5-4l18 78q-23 5-44.5 7.5T480-80ZM170-400h136q-3-20-4.5-40t-1.5-40q0-20 1.5-40t4.5-40H170q-5 20-7.5 40t-2.5 40q0 20 2.5 40t7.5 40Zm34-240h118q9-37 22.5-72.5T376-782q-55 18-99 54.5T204-640Zm172 462q-18-34-31.5-69.5T322-320H204q29 51 73 87.5t99 54.5Zm28-462h152q-12-43-31-82.5T480-798q-26 36-45 75.5T404-640Zm234 0h118q-29-51-73-87.5T584-782q18 34 31.5 69.5T638-640Z" />
      </symbol>      <symbol id="icon-bell" viewBox="0 -960 960 960">
        <path fill="currentColor" d="M160-200v-80h80v-280q0-83 50-147.5T420-792v-28q0-25 17.5-42.5T480-880q25 0 42.5 17.5T540-820v28q80 20 130 84.5T720-560v280h80v80H160Zm320-300Zm0 420q-33 0-56.5-23.5T400-160h160q0 33-23.5 56.5T480-80ZM320-280h320v-280q0-66-47-113t-113-47q-66 0-113 47t-47 113v280Z" />
      </symbol>
      
      
    </svg>
  );
}

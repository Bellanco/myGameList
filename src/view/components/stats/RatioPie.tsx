...
/**
 * Completados frente a abandonados: una CIFRA PROTAGONISTA con la tarta de apoyo.
 *
 * Con dos categorías, el número ES el gráfico —una tarta de dos porciones no dice nada que no diga "80%"—, así
 * que la cifra manda y la figura acompaña para que la proporción se vea de un vistazo. Los porcentajes salen
 * de dentro de las porciones: estaban repitiendo la cifra grande, y en un reparto desigual la porción pequeña
 * no daba para escribirlos sin recortarlos.
 *
 * Cada porción se separa un poco del centro al pasar el ratón y el color sale de los tokens de la paleta, así
 * que la figura se adapta sola a cada tema y a claro/oscuro.
 */
export const RatioPie = memo(function RatioPie({ ratio }: { ratio: StatsSummary['completionRatio'] }) {
  ...
  return (
    <div className="ratio-block">
      <div className="ratio-pie" role="img" aria-label={L.donutAria(percent, completed, abandoned)}>
        <svg viewBox={`0 0 ${SIZE} ${SIZE}`}>
          {drawn.map((slice, index) => (
            ...
            >
              <title>{`${slice.label}: ${formatCount(slice.value)} (${slice.percent}%)`}</title>
              <path d={slice.d} />
            </g>
          ))}
        </svg>
      ...
      <div className="ratio-read">
        <p className="ratio-hero">
          <b>{shown}%</b>

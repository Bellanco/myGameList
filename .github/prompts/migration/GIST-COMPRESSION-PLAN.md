...
> **Estado de implementación (juegos):** Fases 0, 1, 2 y 3 **hechas** en `develop`. Util `src/core/utils/gistCompression.ts` (gzip nativo + base64 troceado, sobre `{enc,payload}` schemaVersion 5). Lectura retrocompatible (`decodeGistContent` en `buildGistReadResponse` y en los merges de overflow). Escritura **gated** por `ENABLE_GAMES_COMPRESSION` (hoy `false`) → cuando esté `true` comprime ancla + chunks + overflow antes de `assertGistSizeWithinLimit`. Tests: `gistCompression.test.ts` (util), `gistCompressionReadWrite.test.ts` (lectura de gists comprimidos con/sin chunks, `readForeignGamesGist`, `enc` desconocido → no rompe, ratio, y escritura comprimida `skipIf(!flag)`); `gistWrite`/`gistV4Cutover`/`gistLegacyUpgrade` hechos robustos al flag (decodifican antes de inspeccionar formato). Verificado con el flag ON: round-trip write→read sin pérdida + idempotente; ratio medido ≈ **31% del original (~69% de ahorro)** sobre datos v4 repetitivos. **Cutover (Fase 4) EJECUTADO: `ENABLE_GAMES_COMPRESSION = true` en develop.** Un gist previo en plano se sigue leyendo (`decodeGistContent` no-op) y da `wasLegacy = true` (`gamesGistWasLegacy`: `!wasCompressed`) → se reescribe comprimido en la siguiente sync (test `gistCompressionReadWrite.test.ts`: "el flip comprime también los chunks SIN CAMBIOS que estaban en plano"). Reversible volviendo a `false`. Precondición asumida: todos los dispositivos ya tenían la LECTURA (Fase 1) desplegada. El social sigue **diferido**.
>
> **Decisión sobre el chunking (punto 8) — IMPLEMENTADO (estimar-y-verificar):** cuando la escritura es comprimida, el nº de chunks se fija por el tamaño **REAL almacenado** (comprimido), no por el JSON plano. `buildGamesFilesForStorage` (en `gistRepository.ts`): (1) mide el ratio real comprimiendo el conjunto sin trocear; si cabe bajo el objetivo (`GAMES_COMPRESSED_TARGET_BYTES = 880 KB`), un solo fichero; (2) estima un presupuesto de plano `objetivo/ratio` (margen 8%); (3) verifica el tamaño comprimido real de cada fichero y encoge+reconstruye si alguno se pasa (converge en 1-2 vueltas). La guarda de 950 KB (`assertGistSizeWithinLimit`) sigue siendo el backstop. Sin compresión, se mantiene el presupuesto plano de 800 KB (conservador: el comprimido nunca supera al plano). `distributeIntoChunks` sigue siendo síncrono y puro; el estimar-y-verificar vive en la capa de escritura porque comprimir es async. Verificado (real x40, 5040 juegos): 4 ficheros (plano) → 2 ficheros (comprimido), round-trip sin pérdida. Test: `gistCompressionReadWrite.test.ts` ("estimar-y-verificar: … menos chunks que el presupuesto plano").

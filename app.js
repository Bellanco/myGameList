...
const GistSync = {
    ...
    async whoami(token) {
        ...
        if (!r.ok)
            throw new Error(`Token inválido (${r.status})`);
    ...
    async read(token, gistId, etag = null) {
        ...
        if (r.status === 304)
            return { notModified: true, etag: r.headers.get('ETag') || etag };
        if (!r.ok)
            throw new Error(`Error al leer el Gist (${r.status})`);
    ...
    async write(token, gistId, data, _etag = null) {
        ...
        if (!r.ok)
            throw new Error(`Error al guardar en Gist (${r.status})`);
...
class SteamListApp {
    ...
    normalize() {
        ...
        let nextId = Math.max(0, ...Object.values(this.data).flat().map(i => i.id)) + 1;
    ...
    _buildSyncPayload() {
        const updatedAt = Number(this.meta?.updatedAt || Date.now());
        return {
    ...
    _extractSyncPayload(raw) {
        ...
        const data = { c: src.c || [], v: src.v || [], e: src.e || [], p: src.p || [] };
        return {
            data,
    ...
    _scheduleRenderTable() {
        if (this._renderTableTimer)
            clearTimeout(this._renderTableTimer);
    ...
    _handleDelegatedAction(e) {
        ...
        if (e.type !== expected && !(e.type === 'dblclick' && el.dataset.dblAction))
            return;
        if (!action)
            return;
        ...
        switch (action) {
            ...
            case 'sync-disconnect': return this._syncDisconnect();
            ...
            case 'toggle-deck':
                el.classList.toggle('active');
                el.classList.toggle('btn-toggle-deck');
                this.renderToolbar();
                return this.renderTable();
    ...
    updateCounters() {
        for (const t of ['c', 'v', 'e', 'p']) {
            ...
            if (cnt)
                cnt.textContent = String(this.data[t].length);
            const tabEl = document.getElementById(`tab-${t}`);
            if (tabEl)
                tabEl.classList.toggle('active', t === this.currentTab);
        }
    }
    switchTab(tab) {
        this.currentTab = tab;
    ...
    syncResponsiveMode() {
        ...
        if (!filtersCompact)
            this._filtersOpen = false;
        else if (this._filtersOpen === null || this._filtersOpen === undefined)
            this._filtersOpen = false;
        return changed;
    }
    ...
    clearFilter(key) {
        ...
        if (key === 'only')
            ...
            el?.classList.remove('active', 'btn-toggle-deck');
        else if (el)
            el.value = '';
        this.renderTable();
        this.renderToolbar();
    ...
    renderActiveFilters(state) {
        ...
        if (state.search)
            chips.push(chip('search', 'Buscar', state.search));
        if (state.genre)
            chips.push(chip('genre', 'Género', state.genre));
        if (state.platform)
            chips.push(chip('platform', 'Plataforma', state.platform));
        if (state.score)
            chips.push(chip('score', 'Punt.', `≥ ${state.score}`));
        if (state.hours) {
        ...
        if (state.only)
            chips.push(chip('only', this.getToolbarBoolLabel(), 'Activado'));
        if (state.deck)
            chips.push(chip('deck', 'Steam Deck', 'Activado'));
        return chips.length
    ...
    renderToolbar() {
      ...
      <div class="filters-row ${compact && this._filtersOpen ? 'open' : ''}">
        ...
        <div class="filter-field filter-field-toggle">
          ...
          <button class="btn btn-toggle ${state.deck ? 'active btn-toggle-deck' : ''}" id="t-deck"
    ...
    `;
        ...
        if (genEl)
            genEl.value = state.genre;
        if (platEl)
            platEl.value = state.platform;
        if (scoreEl)
            scoreEl.value = state.score;
        if (hoursEl)
            hoursEl.value = state.hours;
        this.syncActiveFilters();
    }
    toggleFilters() {
        if (!this.isFiltersCompact())
            return;
        ...
        const filtered = this.data[tab].filter(item => {
            ...
            if (state.hours) {
                ...
                if (state.hours === '5-10' && (hNum <= 5 || hNum > 10))
                    return false;
                if (state.hours === '10-20' && (hNum <= 10 || hNum > 20))
                    return false;
                if (state.hours === '20-40' && (hNum <= 20 || hNum > 40))
                    return false;
                if (state.hours === '40-80' && (hNum <= 40 || hNum > 80))
                    return false;
                if (state.hours === '80-150' && (hNum <= 80 || hNum > 150))
                    return false;
                if (state.hours === '150+' && hNum <= 150)
                    return false;
            }
            return true;
        });
    ...
    sortBy(col) {
        ...
        if (cfg.col === col) {
            ...
            cfg.col = col;
            cfg.asc = !numericCols.includes(col);
        }
        this.renderTable();
    }
    /* ── Tabla ─────────────────────────────────────────────────────── */
        ...
        const fields = [
            ...
            ...tabCfg.detailExtra.map(f => {
                ...
                if (f.hideIfEmpty && !val)
                    return ''; // No renderiza si está vacío y tiene el flag
    ...
    openModal(type, id = null, prefill = null) {
        ...
        if (!id)
            title = prefill ? tabCfg.modalTitles.prefill : tabCfg.modalTitles.new;
    ...
    saveGame() {
        ...
        if (id) {
            ...
            if (idx >= 0)
                this.data[type][idx] = { ...this.data[type][idx], ...payload };
        }
        else {
            payload.id = Math.max(0, ...Object.values(this.data).flat().map(g => g.id)) + 1;
    ...
    deleteAdminTag(value) {
        if (!confirm(`¿Eliminar etiqueta "${value}"?`))
            return;
        const tab = this.currentAdminTab;
        for (const game of Object.values(this.data).flat()) {
            if (tab === 'genres')
    ...
    importData(e) {
        ...
        reader.onload = ev => {
            try {
                ...
                this.notify('Archivo JSON no válido', 'err');
            }
        };
    ...
    async _initSync() {
        ...
        try {
            ...
            if (remoteTs > localTs) {
                ...
                this.meta = { ...(this.meta || {}), updatedAt: remoteTs, etag: remoteEtag, lastRemoteUpdatedAt: remoteTs };
                this._saveLocalState();
                GistSync.saveCfg({ ...(cfg || {}), token: cfg.token, gistId: cfg.gistId, etag: remoteEtag, lastRemoteUpdatedAt: remoteTs });
                this.normalize();
                this.refreshLookups();
                this.render();
                this._setSyncStatus('ok');
                ...
                await this._pushToGist(true, remoteEtag);
            }
            else {
                this.meta = { ...(this.meta || {}), updatedAt: localTs || remoteTs || Date.now(), etag: remoteEtag, lastRemoteUpdatedAt: remoteTs || localTs || 0 };
                this._saveLocalState();
                GistSync.saveCfg({ ...(cfg || {}), token: cfg.token, gistId: cfg.gistId, etag: remoteEtag, lastRemoteUpdatedAt: remoteTs || localTs || 0 });
                this._setSyncStatus('ok');
            }
        }
        catch (err) {
    ...
    async _pushToGist(silent = false, etagOverride = null) {
        ...
        try {
            ...
            const nextCfg = { ...(cfg || {}), token: cfg.token, gistId: cfg.gistId, etag: res?.etag || null, lastRemoteUpdatedAt: payload.updatedAt };
    ...
    _setSyncStatus(status, msg = '') {
        ...
        const labels = { idle: 'No Sincronizado', ok: 'Sincronizado', syncing: 'Sincronizando…', error: 'No Sincronizado' };
        if (lbl)
    ...
    _renderSyncModal() {
        ...
        <div class="sync-section">
          <div class="sync-help">
            <strong>Cómo configurar:</strong><br>Ve a GitHub > Settings > Tokens. Crea uno con permiso <code>gist</code> y pégalo aquí.
          </div>
        </div>
        <div class="sync-section">
          <div class="fg">
            <label class="flabel">Token *</label>
            <div class="token-row"><input class="finput" id="sy-token" type="password" placeholder="ghp_..."><button class="token-toggle" type="button" data-action="toggle-token-visibility">👁</button></div>
          </div>
          <div class="fg">
            <label class="flabel">Gist ID (Vacio la 1ª vez)</label>
            <input class="finput" id="sy-gist" type="text">
          </div>
    ...
    async _syncConnect() {
        ...
        try {
            ...
            if (!gistInput) {
                ...
                this.meta = { ...(this.meta || {}), updatedAt: payload.updatedAt || Date.now(), etag: remote?.etag || null, lastRemoteUpdatedAt: payload.updatedAt || Date.now() };
                this._saveLocalState();
                this.normalize();
                this.refreshLookups();
                this.render();
                this._setSyncStatus('ok');
                this._syncMsg('Conectado y descargado', 'ok');
            }
            setTimeout(() => { this.closeModal('modal-sync'); }, 1400);
        }
        catch (err) {
            this._syncMsg(err.message, 'err');
        }
    }
    async _syncNow() {
        const cfg = GistSync.getCfg();
        if (!cfg)
            return;
        this._syncMsg('Sincronizando…', 'warn');
        try {
            const remote = await GistSync.read(cfg.token, cfg.gistId, cfg.etag || null);
            if (remote?.notModified) {
                // El Gist no ha cambiado desde nuestra última lectura.
                // Comprobamos si tenemos cambios locales que no se han subido.
                const localTs = Number(this.meta?.updatedAt || 0);
                const remoteTs = Number(cfg.lastRemoteUpdatedAt || 0);
                if (localTs > remoteTs) {
                    await this._pushToGist(false);
                    this._syncMsg('Cambios locales subidos', 'ok');
                }
                else {
                    this._syncMsg('Al día', 'ok');
                    this._setSyncStatus('ok');
                }
                return;
            }
            ...
            const remoteEtag = remote?.etag || null;
            if (localTs > remoteTs) {
                // Los datos locales son más recientes → subir al Gist
                await this._pushToGist(false);
                this._syncMsg('Cambios locales subidos', 'ok');
            }
            else {
                // El Gist es más reciente o igual → descargar
                this.data = remoteData.data;
                this.meta = { ...(this.meta || {}), updatedAt: remoteTs || Date.now(), etag: remoteEtag, lastRemoteUpdatedAt: remoteTs || 0 };
                this._saveLocalState();
                this.normalize();
                this.refreshLookups();
                this.render();
                GistSync.saveCfg({ ...(cfg || {}), token: cfg.token, gistId: cfg.gistId, etag: remoteEtag, lastRemoteUpdatedAt: remoteTs || 0 });
                this._setSyncStatus('ok');
                this._syncMsg('Datos actualizados desde Gist', 'ok');
            }
        }
        catch (err) {
            this._syncMsg(err.message, 'err');
